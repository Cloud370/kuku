...
use crate::api::{ApiError, ApiErrorCode, ApiVersion, CredentialSource, CredentialStatus, PlatformCatalog, RevisionToken, TierSummary};
...
impl ConfigService {
    pub async fn open(path: PathBuf, revision: Arc<ServerRevisionCoordinator>) -> Result<Arc<Self>, ApiError> {
        let (raw, last_good, disk_state, disk_digest) = read_state(&path);
        revision
            .register_initial(RevisionDomain::Config, disk_digest.clone().unwrap_or_else(|| accepted_digest(b"missing-config")))
            .await;
        Ok(Arc::new(Self {
            path,
            revision,
    ...
    pub async fn catalog(&self) -> PlatformCatalog {
        let revision = self.revision().await.unwrap_or_else(|_| RevisionToken::parse("0".repeat(64)).expect("valid zero revision"));
        let state = self.inner.read().await;
        let tiers = state
            .last_good
            .as_ref()
        ...
        let credentials = state
                ...
                config
                    ...
                    .map(|(provider_id, provider)| {
                        let (source, environment_reference) = match &provider.credential {
                            ...
                            StoredCredential::EnvironmentReference(name) => (Some(CredentialSource::EnvironmentReference), Some(name.clone())),
                        };
                        CredentialStatus {
            ...
            .unwrap_or_default();
        let default_tier = tiers.iter().find(|tier| tier.is_default).cloned().unwrap_or_else(|| TierSummary {
            tier_id: String::new(), label: String::new(), purpose: String::new(), provider: String::new(), model: String::new(), think: None, is_default: false,
        });
        PlatformCatalog { api_version: ApiVersion, revision, default_tier, tiers, credentials }
    }

    pub async fn commit_config(&self, patch: ConfigPatch, expected: RevisionToken) -> Result<ConfigSnapshot, ApiError> {
        let guard = self.revision.begin(&expected).await?;
        let candidate = patch.file.resolve().map_err(|error| config_error(error.to_string()))?;
        let encoded = toml::to_string_pretty(&patch.file).map_err(|error| config_error(error.to_string()))?;
        ...
        if self.path.exists() {
            ...
            if state.disk_digest.as_ref() != Some(&current_digest) {
                return Err(ApiError::new(ApiErrorCode::Outdated, "config changed on disk", "platform-config"));
            }
        }
        ...
        Ok(ConfigSnapshot { path: self.path.clone(), state: state.disk_state.clone(), raw: state.raw.clone(), resolved: state.last_good.clone() })
    }
    ...
    pub async fn reload_from_disk(&self) -> Result<PlatformState, ApiError> {
        ...
        let mut state = self.inner.write().await;
        let changed = state.disk_digest != disk_digest || state.disk_state != disk_state;
        if changed && matches!(disk_state, PlatformState::Ready | PlatformState::Missing) {
            let digest = disk_digest.clone().unwrap_or_else(|| accepted_digest(b"missing-config"));
        ...
        } else if matches!(disk_state, PlatformState::Invalid { .. }) {
            state.disk_state = disk_state.clone();
        }
        Ok(state.disk_state.clone())
    }
}

fn read_state(path: &Path) -> (Option<ConfigFile>, Option<Arc<Config>>, PlatformState, Option<super::revision::AcceptedDigest>) {
    ...
    let raw = match kuku::config::load_config(path) {
        Ok(raw) => raw,
        Err(error) => return (None, None, PlatformState::Invalid { diagnostics: vec![error.to_string()] }, Some(digest)),
    };
    match raw.resolve() {
        Ok(config) => (Some(raw), Some(Arc::new(config)), PlatformState::Ready, Some(digest)),
        Err(error) => (Some(raw), None, PlatformState::Invalid { diagnostics: vec![error.to_string()] }, Some(digest)),
    }
}
