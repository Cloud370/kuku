use crate::context::{RequestSnapshotBuilder, SnapshotInput};
use crate::error::{Error, Result};
use crate::event::{
    EventPayload, EventStore, ModelStopReason, RequestCause, RequestId, RequestScope,
    RequestStarted, RevisionToken, TaskEvent,
};
...
use super::types::{
    ModelRecoveryInfo, PendingRun, PendingStep, StreamingChunkState, TaskRequestLifecycle, UiEvent,
};
...
async fn dispatch_recovery(
    ...
    if pending.execution_scope.is_some() {
        ...
        let request_scope = RequestScope {
            ...
            request_id: RequestId::derive_for_turn(execution, pending.turn, &request_id).map_err(
                |error| {
                    Error::InvalidEventStream(format!(
                        "cannot derive recovered request id: {error}"
                    ))
                },
            )?,
        };
        ...
        let provider = super::provider::provider_fact(&resolved.kind);
        let tier_id = pending.config.default_tier.clone();
        let catalog_revision = {
            use sha2::Digest;
            RevisionToken::parse(format!(
                "{:x}",
                sha2::Sha256::digest(request.catalog().hash().as_bytes())
            ))
        ...
        let snapshot = RequestSnapshotBuilder::build(SnapshotInput {
            ...
            provider,
            tier_id: &tier_id,
            assembly: request.snapshot_assembly(),
            handoff_context_template: request.snapshot_handoff_template(),
            allowlisted_provider_parameters: request.snapshot_parameters(),
            breakdown: super::provider::request_breakdown_from_assembly(
                request.snapshot_assembly(),
        ...
        super::provider::append_task_request_activity(
            &pending.events_path,
            vec![
        ...
        pending.task_request = Some(TaskRequestLifecycle {
            scope: request_scope,
    ...
    match crate::provider::stream_provider(&resolved, &provider_request, trace).await {
        ...
        Err(failure) => {
            ...
            super::provider::record_task_request_failed(
                &mut pending,
