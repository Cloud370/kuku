...
3. 它追加写入 `model.request`，其中包含解析后的 provider、model 和 provenance。
4. 它将模型响应流式传给 Host，并在完成后追加写入 `model.response`。
5. 如果响应结束这一轮，kuku 会追加写入 `turn.end`。
6. 如果响应请求 Tool，kuku 会追加写入 `tool.call`，评估权限，执行 Tool，追加写入 `tool.result`，然后为下一次模型调用重建上下文。
